"""Game logic and Prolog integration for the Rush Hour puzzle."""
...
except ImportError:  # pragma: no cover - fallback path when pyswip is missing
    Prolog = None

# Typed aliases make the interactions with the search module clearer.
...
class Car:
    """Lightweight container that represents a single vehicle on the board."""
    ...
    def occupy_cells(self) -> Iterable[Tuple[int, int]]:
        """Yield every grid coordinate currently occupied by this car."""
...
class AnimatedCar:
    """Car wrapper with smooth animation support."""
    ...
    def animate(self, speed: float = 0.25) -> bool:
        """Smoothly move towards target. Returns True when still animating."""
...
class Game:
    """Encapsulates the full puzzle state, rendering data, and Prolog bridge."""

    grid_size: int = 6

    def __init__(self, asset_dir: Path, logic_path: Optional[Path] = None) -> None:
        """Initialise an empty game that can later load a concrete level."""
        self.asset_dir = Path(asset_dir)
        self.logic_path = Path(logic_path) if logic_path else Path(__file__).with_name("logic.pl")
    ...
    def load_level(self, file_path: Path) -> None:
        """Read a JSON puzzle definition and populate the in-memory state."""
    ...
    def update_animations(self) -> None:
        """Update all car animations."""
        for animated_car in self.animated_cars.values():
            animated_car.animate()

    def move_car(self, car_id: str, direction: str) -> bool:
        """Attempt to move a car; returns True when the Prolog engine approves."""
        if not self.current_state:
            return False
        for candidate_id, candidate_dir, next_state in self.get_valid_moves(self.current_state):
    ...
    def draw(self, screen) -> None:
        """Render the current board using the supplied Pygame surface with smooth animations."""
        import pygame
        import math

        cell_size = screen.get_height() // self.grid_size

        # Solid base colour so the tiled texture pops.
        screen.fill((32, 34, 40))

        # Draw background image when available by tiling per cell.
        ...
        # Grid lines removed for a seamless tiled background.

        # Exit marker with animation
        exit_y = 2  # Assuming red car is on row 2
        exit_rect = pygame.Rect((self.grid_size - 1) * cell_size, exit_y * cell_size, cell_size, cell_size)
        ...
        # Render cars with smooth animation
        for car_id, animated_car in self.animated_cars.items():
            ...
            if car.identifier.upper() == "R":
                ...
                palette = {
                    0: ((100, 200, 255), (150, 220, 255)),  # Blue
                    1: ((255, 200, 100), (255, 220, 150)),  # Yellow
                    2: ((150, 255, 150), (200, 255, 200)),  # Green
                }
            ...
            if car.length == 3 and self._draw_truck(screen, animated_car, cell_size, car.orientation):
                continue
            ...
            if not rendered_texture:
                # Modern car rendering with gradients and shadows
                shadow_rect = rect.copy()
                shadow_rect.x += 3
                shadow_rect.y += 3
                shadow_surf = pygame.Surface((int(shadow_rect.width), int(shadow_rect.height)), pygame.SRCALPHA)
                pygame.draw.rect(shadow_surf, (0, 0, 0, 80), shadow_surf.get_rect(), border_radius=12)
                screen.blit(shadow_surf, shadow_rect.topleft)

                # Main car body with gradient
                car_surf = pygame.Surface((int(rect.width), int(rect.height)), pygame.SRCALPHA)
                for i in range(int(rect.height)):
                    factor = i / rect.height
                    blend_color = tuple(int(colour[j] * (1 - factor * 0.4) + accent_color[j] * factor * 0.4) for j in range(3))
                    pygame.draw.line(car_surf, blend_color, (0, i), (int(rect.width), i))
                ...
                # Highlight
                highlight = pygame.Surface((int(rect.width), int(rect.height // 3)), pygame.SRCALPHA)
                pygame.draw.rect(highlight, (*accent_color, 100), highlight.get_rect(), border_radius=10)
                screen.blit(highlight, (rect.x, rect.y))

    def apply_solution(self, path: List[MoveTuple]) -> None:
        """Store the solver output so the UI can animate it later."""
        self.solution_path = path
        self.solution_index = 0

    def step_solution(self) -> bool:
        """Advance by one move from the pre-computed solution path."""
    ...
    def reset(self) -> None:
        """Restore the puzzle to the original configuration."""
    ...
    def get_valid_moves(self, state: Optional[StateTuple] = None) -> List[MoveTuple]:
        """Return every legal move according to Prolog or the Python fallback."""
    ...
    # ------------------------------------------------------------------ #
    # Internal helpers hidden from outside callers.
    # ------------------------------------------------------------------ #

    def _initialise_prolog(self) -> None:
        """Load the Prolog knowledge base when pyswip is installed."""
        if Prolog is None:
    ...
    def _cars_to_state(self) -> StateTuple:
        """Convert the dictionary of Car objects into a sorted state tuple."""
        return tuple(
            sorted(
    ...
    def _update_cars_from_state(self, state: StateTuple) -> None:
        """Overwrite the current Car objects using the canonical state tuple."""
        self.cars = {
    ...
    def _load_texture(self, filename: str) -> Optional["pygame.Surface"]:
        """Cache textures from disk so we do not re-load every frame."""
        import pygame
        ...
        if filename not in self._texture_cache:
            ...
            if not path.exists():
                ...
                try:
                    self._texture_cache[filename] = pygame.image.load(str(path)).convert_alpha()
                except pygame.error:
                    self._texture_cache[filename] = None
    ...
    def _blit_texture(
        ...
        """Draw a texture if available, rotating for vertical cars when needed."""
        import pygame
    ...
    def _draw_truck(self, screen, animated_car: AnimatedCar, cell_size: int, orientation: str) -> bool:
        """Render length-3 vehicles using truck head + trailer segments."""
        if self._load_texture("truck_red.png") is None or self._load_texture("trailer.png") is None:
            return False

        import pygame

        offsets = [(0, 0), (1, 0), (2, 0)] if orientation == "H" else [(0, 0), (0, 1), (0, 2)]
    ...
    def _state_to_prolog(self, state: StateTuple) -> str:
        """Serialise a Python state tuple into the Prolog list representation."""
        parts = []
        for identifier, x, y, orientation, length in state:
            parts.append(
    ...
    def _query_prolog_moves(self, state: StateTuple) -> List[MoveTuple]:
        """Ask the Prolog engine for all valid successor states."""
        if not self._prolog:
            return []
    ...
    def _parse_prolog_state(self, state_term) -> StateTuple:
        """Translate a pyswip structure back into the canonical tuple format."""
    ...
    def _python_valid_moves(self, state: StateTuple) -> List[MoveTuple]:
        """Pure Python fallback that mirrors the logic.pl rules - only ONE CELL moves per step."""
        occupied = self._build_occupancy_map(state)
        moves: List[MoveTuple] = []

        for identifier, x, y, orientation, length in state:
            # Remove current car's cells from occupied map for collision checking
            ...
            if orientation == "H":
                # Move left by 1 cell - check if the new leftmost cell is free
                if x - 1 >= 0:
                    new_cell = (x - 1, y)
                ...
                # Move right by 1 cell - check if the new rightmost cell is free
                ...
                # Move up by 1 cell - check if the new topmost cell is free
                if y - 1 >= 0:
                    new_cell = (x, y - 1)
                    if new_cell not in occupied or new_cell in current_car_cells:
                ...
                # Move down by 1 cell - check if the new bottommost cell is free
    ...
    def _build_occupancy_map(self, state: StateTuple) -> Dict[Tuple[int, int], str]:
        """Return a map from occupied cell to the car occupying it."""
    ...
    def _apply_move(
        ...
        """Produce a new state tuple after translating the target car."""
        new_state = []
        for identifier, x, y, orientation, length in state:
